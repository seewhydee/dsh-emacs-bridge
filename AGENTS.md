...
- Session names: `dsh-bridge--session-label` labels a session by its title, else `[Untitled Session]`, and its string arm consults the sessions cache (so a bare id with a cached row still picks up that row's title).  A bare id with no cached row keeps labelling as that id, so a message naming an unknown session stays informative.  `NO-DEFAULT` suppresses the untitled placeholder (returning nil), which is how `dsh-bridge--read-session-id` filters untitled sessions out of completion; they are reached from the DSH-Sessions list instead.  That is why the dispatcher's create verb (`dsh-bridge-create-titled-session`, key `+`) demands a title while the DSH-Sessions `+` binding (`dsh-bridge-create-session` without PROMPT-TITLE) does not.  The DSH-Describe report deliberately keeps its own raw-id/title rendering (`dsh-bridge--describe-insert`) and must not be rerouted through `--session-label`.
- Workspace selection in `dsh-bridge-create-session`: `dsh-bridge--read-workspace` offers the roster as candidates with `completing-read`'s DEF set from `dsh-bridge--workspace-default` (last-active live session's workspace → the buffer's `default-directory` → nil) and REQUIRE-MATCH nil, so RET takes the default, an answer matching one workspace by label or path selects it, and any other answer names a NEW workspace whose directory is then read (an answer that is itself an existing directory skips that prompt; a non-directory answer is a `user-error`).  There is no `"New workspace…"` sentinel.  `dsh-bridge--workspace-payload` turns the `(ID)` / `(nil PATH TITLE)` selection into the route's `{workspaceId}` / `{path, workspaceTitle}` body.  Workspace labels are unique by construction: the plugin refuses create/rename collisions, which is why a typed name resolves unambiguously.
