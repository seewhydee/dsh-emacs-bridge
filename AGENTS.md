...
- Target resolution is host-side, last-active-by-default, with on-demand resume of cold sessions. Preserve the failure semantics: 404 unknown id, 409 subagent-owned or no-active-session, 413 oversize body.
...
- The changed-files fold walks the **raw log**, not the session surface (`tool/call` is log-only). `/turns` carries only `{path, op}`; `GET /changes` is the lazy hunk read. Bounds live in `logic.ts` (`MAX_CHANGED_*`) and are user-visible in README.
