...
- The plugin `inject` list is `['agents', 'webServer', 'sessions', 'sessionPersistence']`. Optional services — `sessionProjectionCache`, `sessionProjections`, `sessionQuery`, `workspaceRegistry`, `agentDefaultModel`, `agentPresets`, `sessionTitle`, `sessionController`, `connection` — are read with `ctx.get(...)` and must tolerate `undefined`: a profile lacking one must still boot and serve a degraded-but-working bridge. Follow the existing pattern: a minimal structural interface, a cast, and a documented fallback. Both lists mirror `src/index.ts`; keep them in sync.
