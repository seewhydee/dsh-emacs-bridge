...
- Session targeting: a command that acts on a session (sends, model changes, answering a question) resolves its target with `dsh-bridge--effective-session` and refuses when that is nil — a nil target is legitimate ("host resolves last-active") for reads and sends, but guessing one for a mutation is not.  `dsh-bridge--prompt-status-session` widens `--effective-session` with the advisory `--last-resolved-active` / `--cache-last-active` caches and is display-only (the prompt header line and its metadata); never use it to choose a target, and do not key the `send-and-exit` resend guard on it.  When `dsh-bridge-answer` has no explicit target, it uses the only pending question if exactly one exists and otherwise refuses.
