...
| `tests/sessions.spec.ts` | create by `path` (new and already-known workspace), create by `workspaceId`, create-argument bounds, workspace rename/conflict/blank/unknown, session rename/archive/unknown, the archived run refusal (409 on `/send` + `/sessions/resume`, read-only `/turns` still serves) and unarchive |
