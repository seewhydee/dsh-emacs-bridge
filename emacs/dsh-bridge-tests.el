  ...
  (let ((dsh-bridge-default-session "s1"))
    ...
    (let ((buf (get-buffer "*dsh-bridge-output*")))
      ...
      (with-current-buffer buf
        ...
        (should (string-match-p " s1" (dsh-bridge--view-header-line)))
...
not the default target."
  (let ((dsh-bridge-default-session "s1"))
    ...
    (with-current-buffer "*dsh-bridge-output*"
      ...
      (should-not (string-match-p " s1" (dsh-bridge--view-header-line)))
      (should-not (string-match-p "target:" (dsh-bridge--view-header-line))))
    (kill-buffer "*dsh-bridge-output*")))
  ...
  (let ((dsh-bridge-default-session "s1"))
    ...
    (let ((buf (get-buffer "*dsh-bridge-output*")))
      ...
      (with-current-buffer buf
        ...
        (should (string-match-p "latest" (dsh-bridge--view-header-line)))))
    (kill-buffer "*dsh-bridge-output*")))
...
POST /fork, then opens the child's view and prompt."
  (let ((dsh-bridge--turns-cache
    ...
    (with-temp-buffer
      ...
      ;; Browsing state keeps the turn refresh off the wire: the seeded cache
...
attaches to a turn that mutated files, as `((path . P) (op . O))' alists."
  (let ((record (list (cons 'turn turn)
    ...
    (when ended-at
      (setq record (append record (list (cons 'endedAt ended-at)
...
(defun dsh-bridge-test--view-file (path &optional op)
  ...
  (list (cons 'path path) (cons 'op (or op "write"))))
  ...
  (let ((dsh-bridge--turns-cache (dsh-bridge-test--view-cache dsh-bridge-test--view-turns))
    ...
    (with-temp-buffer
      ...
      (cl-letf (((symbol-function 'dsh-bridge--request)
        ...
        (should (string-match-p " (2/3)" (dsh-bridge--view-header-line)))
        ;; Older, then at the oldest it stays.
        (dsh-bridge-view-previous-reply)
        (should (equal (buffer-string) dsh-bridge-test--view-oldest-rendered))
        (should (string-match-p " (3/3)" (dsh-bridge--view-header-line)))
        (dsh-bridge-view-previous-reply)
        (should (equal (buffer-string) dsh-bridge-test--view-oldest-rendered))
...
shown turn resets point."
  (let* ((one (dsh-bridge-test--view-turn 7 7000000
    ...
    (with-temp-buffer
      ...
      (dsh-bridge--view-fill "s1" other nil t)
...
render."
  (let* ((seg1 (dsh-bridge-test--view-segment "first" 7001000 1))
    ...
    (with-temp-buffer
      ...
      (goto-char (point-max))
      (dsh-bridge--view-fill "s1" done nil t t)
  ...
  (let* ((seg1 (dsh-bridge-test--view-segment "first" 7001000 1))
    ...
    (with-temp-buffer
      ...
      (cl-labels ((refill (turn)
                    (dsh-bridge--view-fill "s1" turn nil t t)
                    (equal (buffer-string)
...
not the top: the flip is a provenance-mismatch rebuild under follow."
  (let* ((one (dsh-bridge-test--view-turn 7 7000000
    ...
    (with-temp-buffer
      ...
      (setq-local dsh-bridge--view-follow t)
      (goto-char (point-min))
      (dsh-bridge--view-fill "s1" newer nil t t)
...
(ert-deftest dsh-bridge-visit-session-pending-answers ()
  "A pending question sends RET to the answer buffer, before any resume/fetch."
  (let ((answered nil) (ensured nil) (fetched nil))
	(cl-letf (((symbol-function 'dsh-bridge--pending-question)
			   (lambda (id) (and (equal id "s1") '("q1" . nil))))
			  ((symbol-function 'dsh-bridge-answer) (lambda () (setq answered t)))
...
(ert-deftest dsh-bridge-visit-session-running-shows-view ()
  "A running session shows its DSH-View in following state, not a prompt."
  (let ((alist '((sessionId . "s1") (cwd . "/w") (running . t) (turns)))
	...
	(cl-letf (((symbol-function 'dsh-bridge--pending-question) (lambda (_id) nil))
			  ...
			  ((symbol-function 'dsh-bridge--show-session-view)
			   (lambda (buffer) (setq shown buffer) buffer))
			  ((symbol-function 'dsh-bridge--prompt-buffer)
			   (lambda (_id) (setq prompted t) 'prompt-buffer)))
	  (dsh-bridge-test--visit-session "s1"))
	(should (eq shown 'view-buffer))
	(should-not prompted)))

(ert-deftest dsh-bridge-visit-session-open-turn-shows-view ()
  "An idle-status session whose newest turn is still open shows the view."
  (let ((alist '((sessionId . "s1") (cwd . "/w")
				 (turns ((turn . 1) (segments . nil)))))
		(shown nil))
	(cl-letf (((symbol-function 'dsh-bridge--pending-question) (lambda (_id) nil))
			  ((symbol-function 'dsh-bridge--ensure-session-live) (lambda (_id) t))
			  ((symbol-function 'dsh-bridge--session-turns) (lambda (_id) alist))
...
(ert-deftest dsh-bridge-visit-session-empty-opens-prompt ()
  "A session with no output opens a prompt in the same window."
  (let ((alist '((sessionId . "s1") (cwd . "/w") (turns)))
	...
	(cl-letf (((symbol-function 'dsh-bridge--pending-question) (lambda (_id) nil))
			  ...
			  ((symbol-function 'dsh-bridge--show-session-view)
			   (lambda (buffer) (setq shown buffer) buffer)))
	  (dsh-bridge-test--visit-session "s1"))
	(should (eq popped 'prompt-buffer))
	(should-not shown)))

(defconst dsh-bridge-test--complete-turn
  '((turn . 1) (endedAt . 2) (reason . "completed") (segments . nil))
...
(ert-deftest dsh-bridge-visit-session-fetch-failure-stops ()
  ...
  (let ((shown nil) (prompted nil))
	(cl-letf (((symbol-function 'dsh-bridge--pending-question) (lambda (_id) nil))
			  ((symbol-function 'dsh-bridge--ensure-session-live) (lambda (_id) t))
			  ((symbol-function 'dsh-bridge--session-turns) (lambda (_id) nil))
			  ((symbol-function 'dsh-bridge--show-session-view)
			   (lambda (buffer) (setq shown buffer) buffer))
			  ((symbol-function 'dsh-bridge--prompt-buffer)
			   (lambda (_id) (setq prompted t) 'prompt-buffer)))
...
(ert-deftest dsh-bridge-view-for-session-running-placeholder ()
  ...
  (let ((alist '((sessionId . "s1") (cwd . "/w") (running . t) (turns)))
		(waited nil))
	(cl-letf (((symbol-function 'dsh-bridge--view-waiting-fill)
			   (lambda (id base &optional cwd) (setq waited (list id base cwd))))
			  ((symbol-function 'dsh-bridge--view-fill)
			   (lambda (&rest _) (ert-fail "running+empty must use the waiting fill")))
			  ((symbol-function 'dsh-bridge--session-view) (lambda (_id) nil))
			  ((symbol-function 'dsh-bridge--turns-cache-store) (lambda (&rest _) nil)))
...
matches no workspace names a new one."
  (let ((called nil) (table-seen nil))
    ...
    (let ((create (cadr (assoc "/sessions/create"
                               (mapcar (lambda (c) (list (cadr c) c)) called)))))
      (should create)
      (should (equal (cdr (assoc 'workspaceTitle (caddr create))) "Fresh WS"))
...
(ert-deftest dsh-bridge-status-glyph ()
  ...
  (let ((dsh-bridge--session-status nil)
    ...
    (dsh-bridge--status-set "s1" 'idle)
    (should (string= (dsh-bridge--status-glyph "s1") "I")))
  (let ((dsh-bridge--session-status nil)
        (dsh-bridge--sessions-cache nil)
...
from the shown turn's number at rest and while browsing; content without a
turn identity (a pushed message) or a turn no longer cached shows nothing."
  (let ((dsh-bridge--turns-cache (dsh-bridge-test--view-cache dsh-bridge-test--view-turns)))
    (with-temp-buffer
      (dsh-bridge-view-mode)
      (setq-local dsh-bridge--view-content-session "s1")
      ;; The shown turn's number decides: turn 20 is (2/3) at rest ...
      (setq-local dsh-bridge--view-turn 20)
      (should (equal (dsh-bridge--view-turn-position) " (2/3)"))
      ;; ... and mid-browse; turn 30 is (1/3), turn 10 is (3/3).
      (setq-local dsh-bridge--view-browsing t)
      (should (equal (dsh-bridge--view-turn-position) " (2/3)"))
      (setq-local dsh-bridge--view-turn 30)
      (should (equal (dsh-bridge--view-turn-position) " (1/3)"))
      (setq-local dsh-bridge--view-turn 10)
...
(ert-deftest dsh-bridge-view-header-provenance ()
  "The output header's time segment describes the displayed turn.
A pushed message with no turn identity has none; a settled turn shows its
end clock; an open turn shows its elapsed run."
  (let ((dsh-bridge--session-status nil)
        (dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
        (dsh-bridge-status-indicator 'geometric))
    (with-temp-buffer
...
(ert-deftest dsh-bridge-view-header-workspace ()
  "The view header shows the workspace after the session label."
  (let ((dsh-bridge--session-status nil)
        (dsh-bridge--sessions-cache
    ...
    (with-temp-buffer
      (dsh-bridge-view-mode)
      (setq-local dsh-bridge--view-content-session "s1")
      (should (string-match-p "T · myproj" (dsh-bridge--view-header-line)))))
  ;; A session the cache knows but has no workspace for shows no segment.
  (let ((dsh-bridge--session-status nil)
        (dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
        (dsh-bridge-status-indicator 'geometric))
    (with-temp-buffer
...
(ert-deftest dsh-bridge-view-header-truncation ()
  ...
  (let ((dsh-bridge--session-status nil)
        (dsh-bridge--sessions-cache
         '(((id . "s1")
            (title . "A very long session title that would otherwise hide the tail")
            (workspace . "a-long-workspace-name") (live . t))))
        (dsh-bridge-status-indicator 'geometric))
    (with-temp-buffer
      (dsh-bridge-view-mode)
      (setq-local dsh-bridge--view-content-session "s1")
      (setq-local dsh-bridge--view-turn 1)
      (setq-local dsh-bridge--turns-cache
                '(("s1" 1 ((turn . 1) (startedAt . 1000)
...
(ert-deftest dsh-bridge-view-header-format-eval ()
  ...
  (should (eq (car-safe dsh-bridge--view-header-line-format) :eval))
  (should (equal (cadr dsh-bridge--view-header-line-format)
                 '(dsh-bridge--view-header-line (dsh-bridge--header-window-width)))))
...
`dsh-bridge-turn-complete-refetch-waiting-newer'."
  (let ((dsh-bridge--session-status nil)
    ...
    (with-temp-buffer
      ...
      (dsh-bridge--view-fill "s1" nil nil t)
      (should (null dsh-bridge--view-waiting)))))
...
(ert-deftest dsh-bridge-prompt-header-workspace ()
  "The prompt header shows the workspace of the session it names."
  (let ((dsh-bridge--session-status nil)
        (dsh-bridge--sessions-cache
         '(((id . "s1") (title . "T") (workspace . "myproj") (live . t))))
        (dsh-bridge--session-models nil)
    ...
    (with-temp-buffer
      (dsh-bridge-prompt-mode)
      (setq-local dsh-bridge--prompt-session "s1")
      (should (string-match-p "T · myproj" (dsh-bridge--prompt-header-line))))
    ;; A predicted session's qualifier stays adjacent to its name, before the
    ;; workspace.
    (let ((dsh-bridge--prompt-session nil)
          (dsh-bridge--last-resolved-active '("s1" . "T")))
      (with-temp-buffer
        (dsh-bridge-prompt-mode)
...
(ert-deftest dsh-bridge-view-turns-cache-refresh-keeps-browsing-position ()
  "A turn-cache refresh keeps a browsing view on its shown turn: the `(k/n)'
counter is derived from the shown turn's number, so it stays honest when
newer turns arrive."
  (let* ((newest-open (dsh-bridge-test--view-turn 40 4000000
                       (list (dsh-bridge-test--view-segment "newest2" 4001000 1))))
    ...
    (cl-letf (((symbol-function 'dsh-bridge--request)
      ...
      (with-current-buffer (get-buffer-create "*dsh-bridge-output*")
        (dsh-bridge-view-mode)
        (setq-local dsh-bridge--view-content-session "s1")
        ;; Browsing turn 20, at index 1 of the 3-turn list.
        (setq-local dsh-bridge--view-turn 20)
        (setq-local dsh-bridge--view-browsing t)
        (dsh-bridge--view-turns-cache-refresh "s1")
        (should (equal (dsh-bridge--turns-cache-turns "s1") new-turns))
        ;; Turn 20 now sits at index 2 in the refreshed list: (3/4).
        (should (eq dsh-bridge--view-browsing t))
        (should (string-match-p " (3/4)" (dsh-bridge--view-header-line))))
      (kill-buffer "*dsh-bridge-output*"))))
...
it), a browsing view returns to at-rest rather than deriving a position from
an unrelated turn; the buffer text is untouched."
  ...
  (let ((dsh-bridge--turns-cache (dsh-bridge-test--view-cache dsh-bridge-test--view-turns)))
    (cl-letf (((symbol-function 'dsh-bridge--request)
      ...
      (with-current-buffer (get-buffer-create "*dsh-bridge-output*")
        ...
        (setq-local dsh-bridge--view-browsing t)
        (dsh-bridge--view-turns-cache-refresh "s1")
        ;; The entry survives as empty, and the browsing view rests.
...
(ert-deftest dsh-bridge-view-turns-cache-refresh-incremental-keeps-index ()
  "An incremental merge keeps a mid-browse view's `(k/n)' position honest
against the merged list: browsing an older turn, the position is derived
from the shown turn's number."
  (let* ((cached-7 (dsh-bridge-test--view-turn 7 7000000
                    (list (dsh-bridge-test--view-segment "old" 7001000 1))))
         (grown-7 (dsh-bridge-test--view-turn 7 7000000
                   (list (dsh-bridge-test--view-segment "old" 7001000 1)
    ...
    (cl-letf (((symbol-function 'dsh-bridge--request)
      ...
      (with-current-buffer (get-buffer-create "*dsh-bridge-output*")
        (dsh-bridge-view-mode)
        (setq-local dsh-bridge--view-content-session "s1")
        ;; Browsing turn 5, at index 1 of the 2-turn cache.
        (setq-local dsh-bridge--view-turn 5)
        (setq-local dsh-bridge--view-browsing t)
        (dsh-bridge--view-turns-cache-refresh "s1")
        ;; The merged list is [8, 7, 5]; turn 5 derives index 2.
        (should (eq dsh-bridge--view-browsing t))
        (should (string-match-p " (3/3)" (dsh-bridge--view-header-line))))
      (kill-buffer "*dsh-bridge-output*"))))
...
leaves the buffer and browsing state untouched."
  (let ((dsh-bridge--turns-cache (dsh-bridge-test--view-cache dsh-bridge-test--view-turns))
        (msg nil))
    (cl-letf (((symbol-function 'dsh-bridge--request)
               (lambda (_method _path _payload)
...
(ert-deftest dsh-bridge-view-next-reply-auto-follows-at-newest ()
  "M-n onto the newest turn resumes following directly: browsing up from the
  ...
  (let* ((dsh-bridge--turns-cache
    ...
    (cl-letf (((symbol-function 'dsh-bridge--request)
      ...
      (with-current-buffer (get-buffer-create "*dsh-bridge-output*")
        ...
        ;; Mid-browse on the newest-but-one turn: M-n follows.
        (setq-local dsh-bridge--view-turn 20)
        (setq-local dsh-bridge--view-browsing t)
        (dsh-bridge-view-next-reply)
        (should (equal (buffer-string) dsh-bridge-test--view-newest-rendered))
...
it at `(1/X)' (browsing the newest turn, not following); a further M-n turns
following on."
  (let* ((dsh-bridge--turns-cache
          (dsh-bridge-test--view-cache dsh-bridge-test--view-turns))
...
(ert-deftest dsh-bridge-view-follow-enter ()
  "Turn-following shows the newest turn (resetting any mid-browse state) and
...
(ert-deftest dsh-bridge-view-turn-time-label ()
  "The time segment describes the displayed turn, not the session.
An open turn shows its elapsed run (and drives the ticker), a settled turn
shows its end clock, and a pushed message with no turn shows nothing."
  ;; `float-time' is frozen so the rendered duration is exact.
  (cl-letf (((symbol-function 'float-time) (lambda (&rest _) 1065.0)))
    (with-temp-buffer
      (dsh-bridge-view-mode)
      (setq-local dsh-bridge--view-content-session "s1")
      ;; No turn record: a pushed message carries no time segment.
      (should (null (dsh-bridge--view-turn-time-label "s1")))
      (should-not (dsh-bridge--view-running-duration-p))
      ;; An open turn times from its own startedAt, 65s before the frozen now.
      (setq-local dsh-bridge--view-turn 2)
      (setq-local dsh-bridge--turns-cache
      ...
      (let ((dsh-bridge-view-elapsed-ticker nil))
        (should (equal (dsh-bridge--view-turn-time-label "s1") "running"))
        (should-not (dsh-bridge--view-running-duration-p)))
      ;; A settled turn shows its end clock and never ticks.
      (setq-local dsh-bridge--turns-cache
      ...
      (setq-local dsh-bridge--session-status '(("s1" running . 1000000)))
      (setq-local dsh-bridge--view-turn 1)
      (setq-local dsh-bridge--turns-cache
...
round-trip) and no per-view request."
  (let* ((dsh-bridge--turns-cache nil)
    ...
    (with-current-buffer (get-buffer-create "*dsh-bridge-output*")
      ...
      (setq-local dsh-bridge--view-content-session "s1")
      (setq-local dsh-bridge--view-follow t)
      (dsh-bridge--view-fill "s1" before nil t))
    (cl-letf (((symbol-function 'run-at-time)
...
(ert-deftest dsh-bridge-turn-complete-refetch-one-fetch ()
  "The deferred turn-complete refetch fills every shown, non-browsing view
  ...
  (let ((dsh-bridge--session-status nil)
    ...
    (let ((dsh-bridge--turns-cache
      ...
      (with-current-buffer "*dsh-bridge-output*"
        ...
        (should (null dsh-bridge--view-waiting))
        (should (string-match-p "latest/1" (dsh-bridge--view-header-line)))))
    ;; Case 3: a fresh session (waiting for turn 1, no pre-send turn) accepts
    ;; the first committed turn — there is no earlier turn to reject.
    (with-current-buffer "*dsh-bridge-output*"
      (dsh-bridge--view-waiting-fill "s1" nil)
      (should (eq dsh-bridge--view-waiting t))
      (should (equal dsh-bridge--view-turn 1)))
...
buffer is gone); killing the last ticking view cancels the timer."
  (let* ((record (dsh-bridge-test--view-turn
    ...
    (cl-letf (((symbol-function 'dsh-bridge--view-turns-refresh) #'ignore)
      ...
      ;; kill hook that re-evaluates the shared ticker.  The displayed turn
      ;; is open, which is what makes the header tick.
      (with-current-buffer (get-buffer-create "*dsh-bridge-output*")
        (dsh-bridge--view-fill "s1" record)
...
self-retires on the new segment and the earlier body stays byte-identical."
  (let* ((seg1 (dsh-bridge-test--view-segment "first" 7001000 1))
    ...
    (with-temp-buffer
      ...
      (dsh-bridge--view-answer-note-record
       (list (list (cons 'id "q1") (cons 'selected ["Go"]))))
      (dsh-bridge--view-fill "s1" open nil t t)
      (should (string-match-p "You answered" (buffer-string)))
      (setq probe (copy-marker 3))
      (goto-char (point-max))
      (dsh-bridge--view-fill "s1" grown nil t t)
    ...
    (with-current-buffer
      ...
      (cl-letf (((symbol-function 'dsh-bridge--http)
                ...
                ((symbol-function 'message)
                 (lambda (&rest args) (setq msg (apply #'format args)))))
        (dsh-bridge--question-submit))
      (should-not dsh-bridge--question-resolution)
...
(ert-deftest dsh-bridge-prompt-header-attachment-count ()
  "The header never carries an attachment count; the buffer's tags show it."
  (dsh-bridge-test--kill-prompt-buffer)
...
(defun dsh-bridge-test--kill-approval-display ()
  "Kill the shared read-only approval display buffer, if any."
  (when (get-buffer dsh-bridge--approval-buffer-name)
    (kill-buffer (get-buffer dsh-bridge--approval-buffer-name))))
...
silently; the defensive turn-complete clear drops the registry."
...
(ert-deftest dsh-bridge-approval-body-renders-tool-reason-and-detail ()
  "The approval body names the tool, the asker's reason, and the pending tool
call's arguments (pretty-printed JSON)."
  (let ((dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
        (dsh-bridge-approval-answer 'all))
    (let ((body (dsh-bridge--approval-body
                 "s1"
                 (list :tool-name "bash"
                       :call-id "c1"
      ...
      (should (string-match-p "requests approval" body))
      (should (string-match-p "Tool: bash" body))
      (should (string-match-p "need /etc/shadow" body))
      (should (string-match-p "cat /etc/shadow" body))
      ;; The arguments are pretty-printed rather than shown as one long line.
      (should (string-match-p "{\n  \"command\": \"cat /etc/shadow\"\n}" body))
      (should-not (string-match-p "Approval answering is disabled" body)))))

(ert-deftest dsh-bridge-approval-notify-only-shows-and-refuses ()
  "With `dsh-bridge-approval-answer' at `notify-only' the details still render
with a note that answering is disabled, `dsh-bridge-answer' displays them
instead of prompting, and `dsh-bridge--approval-decide' refuses to POST."
  (let ((dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
        (dsh-bridge-approval-answer 'notify-only)
        (dsh-bridge--pending-approvals
         (dsh-bridge-test--pending-approval-registry "s1" "a1"))
        (posted nil) (prompted nil) (shown nil))
    (let ((body (dsh-bridge--approval-body
                 "s1" (list :tool-name "bash" :reason "r" :detail nil))))
      (should (string-match-p "Approval answering is disabled" body)))
    (cl-letf (((symbol-function 'dsh-bridge--approval-show)
               (lambda (session _plist) (setq shown session)))
              ((symbol-function 'dsh-bridge--approval-prompt)
      ...
      (should-not prompted)
      (should-error (dsh-bridge--approval-decide "s1" "a1" "allowed-once" "x")
                    :type 'user-error))
    (should-not posted)))

(ert-deftest dsh-bridge-approval-show-renders-read-only ()
  "The display-only path fills the shared buffer with the request details,
leaves it read-only, and displays it from the top."
  (let ((dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t)))))
    (unwind-protect
        (save-window-excursion
          (dsh-bridge--approval-show
           "s1" (list :tool-name "bash" :reason "need /etc/shadow" :detail nil))
          (with-current-buffer dsh-bridge--approval-buffer-name
      ...
      (dsh-bridge-test--kill-approval-display))))

(ert-deftest dsh-bridge-approval-prompt-shows-details ()
  "The decision prompt shows the request's details immediately, alongside the
three labelled choices."
  (let ((dsh-bridge-approval-answer 'all)
        (dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
        (captured nil))
    (cl-letf (((symbol-function 'read-multiple-choice)
               (lambda (&rest args) (setq captured args) nil)))
      (dsh-bridge--approval-prompt
       "s1" "a1" (list :tool-name "bash" :reason "need it" :detail nil)))
    (should (string-match-p "\\`Approve bash\\? " (nth 0 captured)))
    (should (string-match-p "Tool: bash" (nth 2 captured)))
    (should (eq (nth 3 captured) t))
    (should (equal (mapcar #'car (nth 1 captured)) '(?y ?n ?c)))))

(ert-deftest dsh-bridge-approval-prompt-posts-wire-payloads ()
  "Each decision offered by the prompt POSTs `/dsh-bridge/approval' with the
approval id, session id, and matching decision string, notes the outcome for
the view, and exits to the session's view.  A prompt dismissed without a
choice POSTs nothing."
  (dolist (case '((?y "allowed-once" "You allowed this operation once.")
                  (?n "rejected" "You rejected this operation.")
                  (?c "cancelled" "You cancelled this request.")))
    (let ((dsh-bridge-approval-answer 'all)
          (dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
          (dsh-bridge--view-answer-notes nil)
          (payloads nil)
          (exited nil))
      (cl-letf (((symbol-function 'read-multiple-choice)
                 (lambda (&rest _) (list (nth 0 case) "name" "desc")))
                ((symbol-function 'dsh-bridge--http)
        ...
        (dsh-bridge--approval-prompt
         "s1" "a1"
         (list :tool-name "bash" :call-id "c1" :reason "r" :detail nil)))
      (should (equal (car (car payloads)) "/approval"))
      (should (equal (cdr (assq 'decision (cdr (car payloads)))) (nth 1 case)))
      (should (equal (cdr (assq 'approvalId (cdr (car payloads)))) "a1"))
      (should (equal (cdr (assq 'sessionId (cdr (car payloads)))) "s1"))
      (should (equal exited '(view . "s1")))
      (should (equal (plist-get (cdr (assoc "s1" dsh-bridge--view-answer-notes)) :text)
                     (nth 2 case)))))
  ;; A dismissed prompt (RET, or a quit that returns) decides nothing.
  (let ((dsh-bridge-approval-answer 'all)
        (dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
        (posted nil))
    (cl-letf (((symbol-function 'read-multiple-choice) (lambda (&rest _) nil))
              ((symbol-function 'dsh-bridge--http)
               (lambda (&rest _) (setq posted t) (list nil "{}" 200))))
      (dsh-bridge--approval-prompt "s1" "a1" (list :tool-name "bash" :detail nil)))
    (should-not posted)))
...
response; the decision still reports its own outcome, exits to the view, and
retires the pending approval exactly once."
  (let ((dsh-bridge--pending-approvals
         (dsh-bridge-test--pending-approval-registry "s1" "a1"))
        (dsh-bridge--sessions-cache '(((id . "s1") (title . "T") (live . t))))
        (dsh-bridge--view-answer-notes nil)
        (exited nil) (reported nil))
    (cl-letf (((symbol-function 'message)
               (lambda (fmt &rest args) (setq reported (apply #'format fmt args))))
              ((symbol-function 'dsh-bridge--status-event-render) #'ignore)
              ((symbol-function 'dsh-bridge--view-await-refresh) #'ignore)
              ((symbol-function 'read-multiple-choice)
               (lambda (&rest _) '(?y "yes" "allow")))
              ((symbol-function 'dsh-bridge--view-for-session)
               (lambda (session-id) (cons 'view session-id)))
              ((symbol-function 'dsh-bridge--exit-to-view)
               (lambda (buffer &optional _window) (setq exited buffer)))
      ...
      (dsh-bridge--approval-prompt "s1" "a1" (list :tool-name "bash" :detail nil)))
    (should-not dsh-bridge--pending-approvals)
    (should (equal reported "dsh-bridge: You allowed this operation once."))
    (should (equal exited '(view . "s1")))))

(ert-deftest dsh-bridge-approval-resolved-retires-without-jumping ()
  "An `approval-resolved' frame retires the pending approval and refreshes the
status and view, without moving the user's windows."
  (let ((dsh-bridge--pending-approvals
         (dsh-bridge-test--pending-approval-registry "s1" "a1"))
        (rendered 0) (refreshed 0))
    (cl-letf (((symbol-function 'dsh-bridge--status-event-render)
               (lambda (_id) (cl-incf rendered)))
              ((symbol-function 'dsh-bridge--view-await-refresh)
               (lambda (_id) (cl-incf refreshed)))
              ((symbol-function 'dsh-bridge--exit-to-view)
               (lambda (&rest _) (ert-fail "a resolved frame must not jump"))))
      (dsh-bridge--approval-resolved "s1" "a1" "rejected"))
    (should-not dsh-bridge--pending-approvals)
    (should (equal rendered 1))
    (should (equal refreshed 1))
    ;; A late duplicate frame for the same approval is a harmless no-op.
    (cl-letf (((symbol-function 'dsh-bridge--status-event-render) #'ignore)
              ((symbol-function 'dsh-bridge--view-await-refresh) #'ignore))
      (dsh-bridge--approval-resolved "s1" "a1" "cancelled"))
    (should-not dsh-bridge--pending-approvals)))
...
opt-out."
  (let* ((files (list (dsh-bridge-test--view-file "src/a.ts" "write")
         ...
         (changed (dsh-bridge-test--view-turn
                   ...
                   7002000 nil files))
         (plain (dsh-bridge-test--view-turn
                 7 7000000
                 (list (dsh-bridge-test--view-segment "done" 7001000 1))
                 7002000))
...
(ert-deftest dsh-bridge-view-changed-files-open-turn-order ()
  "For an open turn the footer sits between the body and the terminal
furniture, and the marker stays last; the awaiting note still wins the
furniture slot.  A completed turn shows no awaiting note even while one is
pending (the note belongs to a later, open turn)."
  (let* ((seg (dsh-bridge-test--view-segment "progress" 7001000 1))
    ...
    (with-temp-buffer
      (dsh-bridge-view-mode)
      (setq-local dsh-bridge--view-content-session "s1")
      ;; Open turn: body, footer, marker.
      (let ((suffix (dsh-bridge--view-turn-suffix open "s1")))
        (should (equal (substring-no-properties suffix)
                       (concat "\n\n" footer "\n\n(continuing...)")))
        (should (text-property-any 0 (length suffix)
                                   'dsh-bridge-turn-marker t suffix)))
      ;; A pending question replaces only the marker, after the footer.
      (let ((dsh-bridge--pending-questions
             (dsh-bridge-test--pending-ask "s1" "Approve?")))
        (let ((suffix (dsh-bridge--view-turn-suffix open "s1")))
          (should (equal (substring-no-properties suffix)
                         (concat "\n\n" footer
                                 "\n\n(Awaiting response: press a to view and answer)")))
          (should (text-property-any 0 (length suffix)
                                     'dsh-bridge-awaiting t suffix))
          (should-not (string-match-p "(continuing" suffix))))
      ;; A completed turn drops the awaiting note but keeps the footer.
      (let ((dsh-bridge--pending-questions
             (dsh-bridge-test--pending-ask "s1" "Approve?")))
        (should (equal (substring-no-properties (dsh-bridge--view-turn-suffix done "s1"))
                       (concat "\n\n" footer)))))))
...
footer's visit button resolves a relative path against it at click time."
  (let* ((files (list (dsh-bridge-test--view-file "src/a.ts")))
         (turn (dsh-bridge-test--view-turn
                7 7000000
                (list (dsh-bridge-test--view-segment "done" 7001000 1))
                7002000 nil files))
    ...
    (with-temp-buffer
      (dsh-bridge-view-mode)
      (dsh-bridge--view-fill "s1" turn "/tmp/dsh-session" t)
...
(ert-deftest dsh-bridge-view-changed-files-splice ()
  "An incremental fill replaces the footer with the grown turn's list and keeps
the recorded body/tail lengths in step, leaving a mid-body point alone."
  (let* ((seg1 (dsh-bridge-test--view-segment "first" 7001000 1))
         (seg2 (dsh-bridge-test--view-segment "second" 7002000 2))
         (one (dsh-bridge-test--view-turn
               7 7000000 (list seg1) nil nil
               (list (dsh-bridge-test--view-file "src/a.ts"))))
         (two (dsh-bridge-test--view-turn
               7 7000000 (list seg1 seg2) nil nil
               (list (dsh-bridge-test--view-file "src/a.ts")
                     (dsh-bridge-test--view-file "src/b.ts"))))
         (dsh-bridge--turns-cache (dsh-bridge-test--view-cache (list two one))))
    (with-temp-buffer
      (dsh-bridge-view-mode)
      (dsh-bridge--view-fill "s1" one nil t)
...
(ert-deftest dsh-bridge-view-changed-files-pushed-message ()
  "A pushed message has no turn identity, so it never renders a footer."
  (should (equal (dsh-bridge--view-turn-suffix nil "s1") ""))
  (let ((dsh-bridge--turns-cache nil))
    (with-temp-buffer
      (dsh-bridge-view-mode)
...
cache — the footer's attribution source."
  (let* ((json (concat "{\"sessionId\":\"s1\",\"epoch\":0,"
                       "\"turns\":[{\"turn\":2,\"startedAt\":1000,"
                       "\"endedAt\":2000,\"reason\":\"completed\","
                       "\"segments\":[{\"text\":\"done\",\"time\":1500,\"step\":1}],"
                       "\"files\":[{\"path\":\"src/a.ts\",\"op\":\"write\"}]}]}"))
         (dsh-bridge--turns-cache nil))
    (cl-letf (((symbol-function 'dsh-bridge--request)
               (lambda (_m _p _payload)
                 (cons 200 (dsh-bridge--parse-json-body json)))))
      (dsh-bridge--turns-cache-fetch "s1"))
    (let* ((turn (car (dsh-bridge--turns-cache-turns "s1")))
           (file (car (alist-get 'files turn))))
